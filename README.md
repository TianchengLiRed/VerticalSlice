...
### Visual Scripting
<img width="1204" height="813" alt="截屏2026-05-14 22 36 37" src="https://github.com/user-attachments/assets/2a737be4-53c9-4699-930d-ffba1cc4a915" />
...
Question2: Based on the playtest feedback, of more visualization, UI and need more clear tutorial. I mainly improved the visualization mechanics of the game. Implemented that the player changes direction when moving, the magazine UI to visualize ammo and the mode UI to clarify the player's current mode. In the move mode, the plane's color will change when the player hovers over it. Implemented camera shaking when the player is attacked by the ghost. Added Ghost's icon when attacked by a player so player wont be confused about who got attacked based on playtest feedback that someone confuse about. Polished tutorial sheet. Implemented the camera zooming function and the camera follow player function.  Added level choosing mechanic. And bullets will now be blocked by walls.
